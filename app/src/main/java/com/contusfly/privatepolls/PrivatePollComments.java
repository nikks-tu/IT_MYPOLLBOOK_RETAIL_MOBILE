...
public class PrivatePollComments extends Activity implements EndLessListView.EndlessListener {
    ...
    public void initViews() {
        ...
        adapter = new PrivatePollCommentsAdapter(pollCommentsActivity, dataResults, R.layout.activity_comments_singleview, userPollId, userPollId, userCommentsCountPosition);
        //Set bottom footer view
        list.setLoadingView(R.layout.layout_loading);
        //Sets the data behind this ListView.
        list.setPrivateCommentsPoll(adapter);
        //Setting the lisner
    ...
    private void commentsPollDisplay(CommentDisplayResponseModel commentsPollDisplay) {
        ...
        if (!dataResults.isEmpty()) {
            ...
            if (Integer.parseInt(getCurrentPage) == 1) {
                ...
                // The Adapter provides access to the data items.
                // The Adapter is also responsible for making a View for each item in the data set.
                adapter = new PrivatePollCommentsAdapter(pollCommentsActivity, dataResults, R.layout.activity_comments_singleview, userPollId, userId, userCommentsCountPosition);
                //Set bottom footer view
                list.setLoadingView(R.layout.layout_loading);
                //Sets the data behind this ListView.
                list.setPrivateCommentsPoll(adapter);
                ...
                list.addPrivatePollCommentsData(dataResults);
            }
    ...
    private void pollCommentsAddResponse(CommentDisplayResponseModel comments) {
        ...
        if (comments.getResults().getData().isEmpty()) {
            ...
            list.addPrivatePollCommentsData(mDataResults);
