 ...
 */
public class UserPollCustomAdapter extends ArrayAdapter<UserPollResponseModel.Results.Data> implements CompoundButton
    ...
    private UserPollResponseModel.Results.Data userPollResponse;
    ...
    private View.OnClickListener mQuestionImage1 = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            //Question image
            pollImage1 = userPollResponse.getPollquestionImage().replaceAll(" ", "%20");
    ...
    private View.OnClickListener mQuestionImage2 = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            userPollResponse = getItem(clickPosition);
            //Question image
            pollImage2 = userPollResponse.getPollquestionImage1().replaceAll(" ", "%20");
            //This method will be called wto show th eimage in full view
    ...
    private View.OnClickListener mSingleOption = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            userPollResponse = getItem(clickPosition);
            //Question image
            if (("").equals(userPollResponse.getPollquestionImage1())) {
                image = userPollResponse.getPollquestionImage().replaceAll(" ", "%20");
            } else {
                image = userPollResponse.getPollquestionImage1().replaceAll(" ", "%20");
            }
            //This method will be called wto show th eimage in full view
     ...
     * @param list
     */
    public UserPollCustomAdapter(Activity activity, List<UserPollResponseModel.Results.Data> dataResults, int
    ...
    private void imageQuestionClickAction(int clickPosition, String pollImageMyPoll) {
        ...
        MApplication.setString(userPollActivity, Constants.DATE_UPDATED, MApplication
                .getTimeDifference(userPollResponse.getUpdatedAt()));
        ...
        MApplication
                .setString(userPollActivity, Constants.CAMPAIGN_CATEGORY, userPollResponse.getCategory().getCategory()
                                                                                          .getCategoryName());
        //Setting the logo in prefernce
        Log.i("ImageCLick", "imageQuestionClickAction: " + userPollResponse.getUserInfo().getUserProfileImg());
        MApplication.setString(userPollActivity, Constants.CAMPAIGN_LOGO, userPollResponse.getUserInfo()
                                                                                          .getUserProfileImg());
        //Moving from one activity to another activity
        if (userPollResponse.getCategory().getCategory().getCategoryName().equalsIgnoreCase("Admin")) {
            isAdmin = true;
        }
    ...
    public View getView(final int position, View mView, ViewGroup parent) {
        ...
        if(userPollResponse!=null) {
            idRefrenceView = userPollResponse.getPollType();
            //Poll question from the response
            pollQuestion = userPollResponse.getPollQuestion();
            ...
            if(pollAnswer1!=null)
            pollAnswer1 = userPollResponse.getUserPollsAns().get(0).getPollAnswer1();
            //poll answer form the response
            if(pollAnswer2!=null)
            pollAnswer2 = userPollResponse.getUserPollsAns().get(0).getPollAnswer2();
            //poll answer form the response
            if(pollAnswer3!=null)
            pollAnswer3 = userPollResponse.getUserPollsAns().get(0).getPollAnswer3();
            //poll answer form the response
            if(pollAnswer4!=null)
            pollAnswer4 = userPollResponse.getUserPollsAns().get(0).getPollAnswer4();
            //youtube usrl from tthe response
            youTubeUrl = userPollResponse.getYouTubeUrl();
    ...
    private void clickAction(int clickPosition) {
        ...
        pollType = userPollResponse.getPollType();
        //Poll id from the response
        String pollId = userPollResponse.getId();
        if (!preferenceUserPollIdAnswer.isEmpty()) {
            if (preferenceUserPollIdAnswer.contains(userPollResponse.getId())) {
                ...
                MApplication.setString(userPollActivity, Constants.CAMPAIGN_NAME, userPollResponse.getUserInfo()
                                                                                                  .getUserName());
                //Setting the categoryname in prefernce
                MApplication.setString(userPollActivity, Constants.CAMPAIGN_CATEGORY, userPollResponse.getCategory()
                                                                                                      .getCategory()
                                                                                                      .getCategoryName());
    ...
    private void validateViewLayoutOne(final ViewHolderLayoutOne holder, final int position) {
        ...
        if (preferenceUserPollIdAnswer.contains(userPollResponse.getId())) {
            ...
            int answeredPosition = preferenceUserPollIdAnswer.indexOf(userPollResponse.getId());
        ...
        if (!userPollResponse.getCategory().getCategory().getCategoryName().equals("Admin") &&userPollResponse.getUserInfo()!=null&& userPollResponse.getUserInfo().getUserProfileImg()!=null) {
            Utils.loadImageWithGlide(userPollActivity, userPollResponse.getUserInfo().getUserProfileImg()
        ...
        holder.radioNoUserPoll.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View view, MotionEvent motionEvent) {
                ...
                if (!view.isClickable()) {
                    ...
                    //poll answer
                    pollAnswer = userPollResponse.getUserPollsAns().get(0).getPollAnswer2();
        ...
        holder.radioYesUserPoll.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View view, MotionEvent motionEvent) {
                ...
                if (!view.isClickable()) {
                    ...
                    //poll answer
                    pollAnswer = userPollResponse.getUserPollsAns().get(0).getPollAnswer1();
                    //Option selection request
                    optionSelection(userPollResponse, clickPosition, holderUserView1, null, null, null);
                }
                return false;
    ...
    private void validateViewLayoutTwo(final ViewHolderLayoutTwo holder, final int position) {
        ...
        if (("").equals(pollAnswer3)) { //If the poll answer option is empty then the view will invisible
            ...
            if (("").equals(pollAnswer4)) {   //If the poll answer option is empty then the view will invisible
                holder.radioOptions4MultipleOptionsUserPoll.setVisibility(View.GONE);
                    ...
                    .equals(pollAnswer4)) { //If the poll answer option is not empty then the view will invisible
        ...
        holder.txtNameMultipleOptionsUserPoll
                .setText(MApplication.getDecodedString(userPollResponse.getUserInfo().getUserName()));
        ...
        holder.txtCategoryMultipleOptionsUserPoll
                .setText(userPollResponse.getCategory().getCategory().getCategoryName());
        //Setting the user profile image
        if (!userPollResponse.getCategory().getCategory().getCategoryName().equalsIgnoreCase("Admin") && userPollResponse.getUserInfo().getUserProfileImg()!=null) {
            Utils.loadImageWithGlide(userPollActivity, userPollResponse.getUserInfo().getUserProfileImg()
                                                                       .replaceAll(" ", "%20"), holder
        ...
        holder.radioOptions3MultipleOptionsUserPoll.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View view, MotionEvent motionEvent) {
                ...
                if (!view.isClickable()) {
                    ...
                    //poll answer
                    pollAnswer = userPollResponse.getUserPollsAns().get(0).getPollAnswer3();
                    //Option selection request
                    optionSelection(userPollResponse, clickPosition, null, holder, null, null);
                }
                return false;
        ...
        holder.radioOptions4MultipleOptionsUserPoll.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View view, MotionEvent motionEvent) {
                ...
                if (!view.isClickable()) {
                    ...
                    //poll answer
                    pollAnswer = userPollResponse.getUserPollsAns().get(0).getPollAnswer4();
                    //Option selection request
                    optionSelection(userPollResponse, clickPosition, null, holder, null, null);
                }
                return false;
...
//Interface definition for a callback to be invoked when a view is clicked.
    ...
    private void validateViewLayoutThree(final ViewHolderLayoutThree holder, final int position) {
        ...
        if (preferenceUserPollIdAnswer.contains(userPollResponse.getId())) {
            ...
            //If the value matches then the radio option willbec cheked ,text color will be black and clickable will
            // be false.
            if (userPollResponse.getUserPollsAns().get(0).getPollAnswer1().equals(value)) {
                holder.radioOptions1PhotoComparisonUserPoll.setChecked(true);
            } else if (userPollResponse.getUserPollsAns().get(0).getPollAnswer2().equals(value)) {
                holder.radioOptions2PhotoComparisonUserPoll.setChecked(true);
            } else if (userPollResponse.getUserPollsAns().get(0).getPollAnswer3().equals(value)) {
                holder.radioOptions3PhotoComparisonUserPoll.setChecked(true);
            } else if (userPollResponse.getUserPollsAns().get(0).getPollAnswer4().equals(value)) {
        ...
        holder.txtCategoryPhotoComparisonUserPoll
                .setText(userPollResponse.getCategory().getCategory().getCategoryName());
        //Setting the user profile image
        if (!userPollResponse.getCategory().getCategory().getCategoryName().equals("Admin") && userPollResponse.getUserInfo().getUserProfileImg()!=null )
            Utils.loadImageWithGlide(userPollActivity, userPollResponse.getUserInfo().getUserProfileImg().replaceAll(" ", "%20"), holder.imgProfilePhotoComparisonUserPoll, R.drawable.img_ic_user);
        else {
    ...
    private void holder3ViewOnClick(final ViewHolderLayoutThree holder, final int position) {
        ...
        holder.imageAnswer1PhotoComparisonUserPoll.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                userPollResponse = getItem(clickPosition);
                //poll answer url
                pollAnswer1 = userPollResponse.getUserPollsAns().get(0).getPollAnswer1().replaceAll(" ", "%20");
        ...
        holder.imageAnswer2PhotoComparisonUserPoll.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                userPollResponse = getItem(clickPosition);
                //poll answer url
                pollAnswer2 = userPollResponse.getUserPollsAns().get(0).getPollAnswer2().replaceAll(" ", "%20");
                //Calling a method to on click
        ...
        holder.imageAnswer3PhotoComparisonUserPoll.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                userPollResponse = getItem(clickPosition);
                //poll answer url
                pollAnswer3 = userPollResponse.getUserPollsAns().get(0).getPollAnswer3().replaceAll(" ", "%20");
                //Calling a method to on click
        ...
        holder.imageAnswer4PhotoComparisonUserPoll.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                userPollResponse = getItem(clickPosition);
                //poll answer url
                pollAnswer4 = userPollResponse.getUserPollsAns().get(0).getPollAnswer4().replaceAll(" ", "%20");
                //Calling a method to on click
        ...
        holder.radioOptions2PhotoComparisonUserPoll
                .setOnCheckedChangeListener(new RadioButton.OnCheckedChangeListener() {
                    ...
                    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                        ...
                        //poll answer url
                        pollAnswerSelectedId = "2";
                        pollAnswer = userPollResponse != null ? userPollResponse.getUserPollsAns().get(0).getPollAnswer2() : null;
                        optionSelection(userPollResponse, clickPosition, null, null, holder, null);
                    }
        ...
        holder.radioOptions3PhotoComparisonUserPoll
                .setOnCheckedChangeListener(new RadioButton.OnCheckedChangeListener() {
                    ...
                    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                        ...
                        //poll answer url
                        pollAnswerSelectedId = "3";
                        pollAnswer = userPollResponse != null ? userPollResponse.getUserPollsAns().get(0).getPollAnswer3() : null;
                        optionSelection(userPollResponse, clickPosition, null, null, holder, null);
                    }
...
//
     ...
     */
    private void optionSelection(final UserPollResponseModel.Results.Data userPollResponse, final int clickPosition,
        ...
        String pollId = this.userPollResponse.getId();
        //Getting the poll answer id from the response
        String pollAnswerId = this.userPollResponse.getUserPollsAns().get(0).getId();
        PollParticipateRestClient.getInstance()
                                 .postParticipateApi("polls_participate", userId, pollId, pollAnswerId, pollAnswer, pollAnswerSelectedId, new String("1")
                                         , new Callback<PollParticipateResponseModel>() {
                                             ...
                                             public void success(PollParticipateResponseModel
                                                 ...
                                                 if (("1").equals(pollParticipateResponseModel.getSuccess())) {
                                                     ...
                                                     if (("1").equals(userPollResponse.getPollType())) {
                                                         Log.e("test", "test");
                                                     ...
                                                     } else if (("2").equals(userPollResponse.getPollType())) {
                                                         ...
                                                         holder2.txtCountsMultipleOptionsUserPoll.setText(String
                                                                 .valueOf(preferenceUserPollParticipateCount
                                                                         .get(clickPosition)));
                                                     } else if (("3").equals(userPollResponse.getPollType())) {
                                                         //Setting the clickable as false
                                                         ...
                                                         holder3.txtCountPhotoComparisonUserPoll.setText(String
                                                                 .valueOf(preferenceUserPollParticipateCount
                                                                         .get(clickPosition)));
                                                     } else if (("4").equals(userPollResponse.getPollType())) {
    ...
    private void commentClickAction(int clickPosition) {
        //response from the server
        userPollResponse = getItem(clickPosition);
        String id = userPollResponse.getId();
        //Moving from one activity to another activity
        Intent details = new Intent(userPollActivity, PollComments.class);
        //Passing the value from one activity to another
        details.putExtra(Constants.POLL_ID, id);
        //Passing the value from one activity to another
        details.putExtra(Constants.POLL_TYPE, userPollResponse.getPollType());
        //Passing the value from one activity to another
